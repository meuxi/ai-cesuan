...
移植自 MingAI-master/src/lib/fortune-interpretations.ts
"""

from typing import List, Dict, Literal

InterpretationMode = Literal['colloquial', 'professional', 'technical']

# 十神解读模板
TEN_GOD_INTERPRETATIONS: Dict[str, Dict[InterpretationMode, str]] = {
    '比肩': {
...
# 运势分数解读
SCORE_INTERPRETATIONS: Dict[str, Dict[InterpretationMode, str]] = {
    'high': {
...
# 各维度建议
DIMENSION_ADVICE: Dict[str, Dict[str, Dict[InterpretationMode, str]]] = {
    'career': {
        'high': {
...
# 关键日期描述
KEY_DATE_DESCRIPTIONS: Dict[str, Dict[InterpretationMode, str]] = {
    '大吉日': {
...
def get_ten_god_interpretation(ten_god: str, mode: InterpretationMode = 'colloquial') -> str:
    """根据模式获取十神解读"""
    return TEN_GOD_INTERPRETATIONS.get(ten_god, TEN_GOD_INTERPRETATIONS['比肩']).get(mode, '')


def get_score_interpretation(score: int, mode: InterpretationMode = 'colloquial') -> str:
    """根据模式获取分数解读"""
    level = get_score_level(score)
    return SCORE_INTERPRETATIONS.get(level, {}).get(mode, '')


def get_dimension_advice(
    dimension: str,
    score: int,
    mode: InterpretationMode = 'colloquial'
) -> str:
    """根据模式获取维度建议"""
    level = get_score_level(score)
    return DIMENSION_ADVICE.get(dimension, {}).get(level, {}).get(mode, '')


def get_key_date_description(desc: str, mode: InterpretationMode = 'colloquial') -> str:
    """根据模式获取关键日期描述"""
    return KEY_DATE_DESCRIPTIONS.get(desc, {}).get(mode, desc)


def generate_fortune_interpretation(
    ten_god: str,
    scores: Dict[str, int],
    mode: InterpretationMode = 'colloquial'
) -> List[str]:
    ...
    Args:
        ten_god: 十神
        scores: 各维度分数 {'overall', 'career', 'love', 'wealth', 'health', 'social'}
        mode: 解读模式
    ...
    # 十神总述
    interpretation.append(get_ten_god_interpretation(ten_god, mode))
    
    # 综合运势
    interpretation.append(get_score_interpretation(scores.get('overall', 70), mode))
    
    # 各维度建议（只添加分数较高或较低的）
    dimensions = [
        ('career', scores.get('career', 70)),
        ('wealth', scores.get('wealth', 70)),
        ('love', scores.get('love', 70)),
    ...
    if highest[1] >= 75:
        interpretation.append(get_dimension_advice(highest[0], highest[1], mode))
    
    if lowest[1] < 65 and lowest[0] != highest[0]:
        interpretation.append(get_dimension_advice(lowest[0], lowest[1], mode))
    
    return [i for i in interpretation if i]
