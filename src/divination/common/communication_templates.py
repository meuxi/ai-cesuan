...
数据来源：MingAI src/lib/communication-templates.ts
"""
from typing import Dict, List, TypedDict, Literal


class TriggerFactor(TypedDict):
    """冲突触发因素"""
    scenario: str      # 触发情境
    trigger: str       # 触发行为
    warning: str       # 警示信息
...
class CommunicationTemplate(TypedDict):
    """沟通建议模板"""
    title: str
    context: str           # 适用情境
    script: str            # 沟通话术
    tips: List[str]        # 沟通技巧
    avoid_phrases: List[str]  # 避免用语


# 冲突类型
ConflictType = Literal['五行相克', '五行被克', '日支相冲', '年支相冲', 'default']

# 关系类型
RelationType = Literal['love', 'business', 'family']


# 冲突触发因素配置
CONFLICT_TRIGGERS: Dict[str, List[TriggerFactor]] = {
    '五行相克': [
        {
            'scenario': '重大决策时刻',
...
# 沟通建议模板配置
COMMUNICATION_TEMPLATES: Dict[str, Dict[str, CommunicationTemplate]] = {
    'love': {
        '五行相克': {
            'title': '克制与尊重的平衡',
...
# 严重程度建议
SEVERITY_ADVICE: Dict[str, List[str]] = {
    'high': [
        '建议寻求专业婚姻/家庭咨询师的帮助',
        '设定冷静期，避免在情绪激动时做决定',
        '制定明确的冲突处理规则，双方共同遵守',
...
def get_conflict_triggers(conflict_type: str) -> List[TriggerFactor]:
    """获取冲突触发因素"""
    return CONFLICT_TRIGGERS.get(conflict_type, CONFLICT_TRIGGERS['default'])
...
def get_communication_template(
    ...
    """获取沟通建议模板"""
    templates = COMMUNICATION_TEMPLATES.get(relation_type, COMMUNICATION_TEMPLATES['love'])
    return templates.get(conflict_type, templates['default'])


def get_severity_advice(severity: str) -> List[str]:
    """根据冲突严重程度获取通用建议"""
    return SEVERITY_ADVICE.get(severity, SEVERITY_ADVICE['low'])


def get_all_conflict_types() -> List[str]:
    """获取所有冲突类型"""
    return ['五行相克', '五行被克', '日支相冲', '年支相冲']


def get_all_relation_types() -> List[str]:
    """获取所有关系类型"""
    return ['love', 'business', 'family']
